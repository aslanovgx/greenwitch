...
function ProductCardComponent({
  ...
  let badge: "ENDİRİMLİ" | "YENİ" | "BESTSELLER" | null = null;
  ...
  if (isMostSales && item.bestSeller) {
    ...
    badge = item.bestSeller ? "BESTSELLER" : null;
  } else {
    // all
  ...
  // 4) Admin rəngi
  const colorByBadge: Record<"BESTSELLER" | "YENİ" | "ENDİRİMLİ", string | undefined> = {
    BESTSELLER: colors[1],     // 1 → BEST
    YENİ: colors[2],      // 2 → NEW
    ENDİRİMLİ: colors[3],  // 3 → ENDİRİM
  };
  const badgeColor = badge ? colorByBadge[badge as "BESTSELLER" | "YENİ" | "ENDİRİMLİ"] : undefined;
